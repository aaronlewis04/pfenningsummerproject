...
IA10plot1 <- VlnPlot(IA10, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) 
...
IA10plot5 <- DimPlot(IA10, reduction = "umap")
...
IA10plot6 <- DimPlot(IA10, reduction = "umap", label = TRUE, pt.size = 0.5, repel = TRUE) #new UMAP clustering with celltypes
...
IA10 <- FindClusters(IA10) #resetting things so it is 
names(IA10GOclusternames) <- levels(IA10) 
IA10 <- RenameIdents(IA10, IA10GOclusternames) 
IA10plot7 <- DimPlot(IA10, reduction = "umap", label = TRUE, pt.size = 0.5, repel = TRUE)
