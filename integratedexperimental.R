...
temp <- FindMarkers(rada, ident.1 = "NA-9", ident.2 = "I-A10") #differenital expression on the two datasets. 106 diff. exp. genes

temp$gene <- rownames(temp)
library(ggrepel)
#creating a volcano plot from the differential expression results 
p5 <- ggplot(data=temp, aes(x=avg_log2FC, y=-log10(p_val_adj))) +
  geom_point() + 
  theme_minimal() +
  geom_text_repel(data = temp[1:10,],aes(x = avg_log2FC, y = -log10(p_val_adj),label=gene, max.overlaps = 20)) +
...
p6 <- VlnPlot(rada, features = c("Ube3a"), split.by = "Sample", group.by = "number.celltype",
